...
object TripleDes {
    ...
    fun keySetup(key: ByteArray, mode: Int): Array<Array<IntArray>> {
    ...
    private fun keySchedule(key: ByteArray, mode: Int): Array<IntArray> {
        ...
        for (i in 0 until 16) {
            c = ((c shl KEY_RND_SHIFT[i]) or (c ushr (28 - KEY_RND_SHIFT[i]))) and 0xFFFFFFF0.toInt()
            d = ((d shl KEY_RND_SHIFT[i]) or (d ushr (28 - KEY_RND_SHIFT[i]))) and 0xFFFFFFF0.toInt()
            ...
            for (j in 24 until 48) {
                schedule[togen][j / 8] = schedule[togen][j / 8] or bitnumIntr(d, KEY_COMPRESSION[j] - 27, 7 - (j % 8))
            }
        }
    ...
    fun crypt(inputData: ByteArray, key1: Array<IntArray>, key2: Array<IntArray>, key3: Array<IntArray>): ByteArray {
        var data = inputData
        data = singleCrypt(data, key1)
        data = singleCrypt(data, key2)
        data = singleCrypt(data, key3)
        return data
    }
    
    private fun singleCrypt(inputData: ByteArray, key: Array<IntArray>): ByteArray {
        val (s0, s1) = initialPermutation(inputData)
        
        var s0Var = s0
        var s1Var = s1
        
        for (idx in 0 until 15) {
            val previousS1 = s1Var
            s1Var = f(s1Var, key[idx]) xor s0Var
            s0Var = previousS1
        }
        s0Var = f(s1Var, key[15]) xor s0Var
        
        return inversePermutation(s0Var, s1Var)
    }
    ...
    private fun f(state: Int, key: IntArray): Int {
        ...
        for (i in lrgstate.indices) {
            lrgstate[i] = lrgstate[i] xor key[i]
        }
        
        var newState = ((SBOX[0][sboxBit(lrgstate[0] ushr 2)] shl 28) or
        ...
        newState = (bitnumIntl(newState, 15, 0) or bitnumIntl(newState, 6, 1) or
                bitnumIntl(newState, 19, 2) or bitnumIntl(newState, 20, 3) or
                bitnumIntl(newState, 28, 4) or bitnumIntl(newState, 11, 5) or
                bitnumIntl(newState, 27, 6) or bitnumIntl(newState, 16, 7) or
                bitnumIntl(newState, 0, 8) or bitnumIntl(newState, 14, 9) or
                bitnumIntl(newState, 22, 10) or bitnumIntl(newState, 25, 11) or
                bitnumIntl(newState, 4, 12) or bitnumIntl(newState, 17, 13) or
                bitnumIntl(newState, 30, 14) or bitnumIntl(newState, 9, 15) or
                bitnumIntl(newState, 1, 16) or bitnumIntl(newState, 7, 17) or
                bitnumIntl(newState, 23, 18) or bitnumIntl(newState, 13, 19) or
                bitnumIntl(newState, 31, 20) or bitnumIntl(newState, 26, 21) or
                bitnumIntl(newState, 2, 22) or bitnumIntl(newState, 8, 23) or
                bitnumIntl(newState, 18, 24) or bitnumIntl(newState, 12, 25) or
                bitnumIntl(newState, 29, 26) or bitnumIntl(newState, 5, 27) or
                bitnumIntl(newState, 21, 28) or bitnumIntl(newState, 10, 29) or
    ...
    private fun bitnum(a: ByteArray, b: Int, c: Int): Int {
        return (((a[(b / 32) * 4 + 3 - (b % 32) / 8].toInt() ushr (7 - b % 8)) and 1) shl c)
    }
